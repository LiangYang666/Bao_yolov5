...
def detect(save_img=False):
    ...
    save_dir = Path(increment_path(Path(opt.data_dir) / 'yolov5_rundata' / 'dect' / (opt.name + f'_{save_name_add}_'),
                                   exist_ok=opt.exist_ok))  # increment run

    # save_dir = Path(increment_path(Path(opt.data_dir) / Path(opt.project) / opt.name, exist_ok=opt.exist_ok))  # increment run
    (save_dir / 'labels_detect' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir
...
if __name__ == '__main__':
    ...
    parser.add_argument('--brand', default='Chanel', help='save results to project/name')
    parser.add_argument('--part', default='sign', help='save results to project/name')
    ...
    parser.add_argument('-b', '--batch-size', type=int, default=10, help='size of each image batch')
    # parser.add_argument('-test', '-test_txtl_file', dest='test_txt',
    #                     type=str, default='testby0.txt', help="test txt file")
    opt = parser.parse_args()

    opt.data_dir = f'/media/D_4TB/YL_4TB/BaoDetection/data/{opt.brand}/LetterDetection/data/{opt.part}'
    print(opt)
