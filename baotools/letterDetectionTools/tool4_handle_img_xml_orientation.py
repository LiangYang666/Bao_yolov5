...
if __name__ == "__main__":
    src_img_dir = f'/media/D_4TB/YL_4TB/BaoDetection/data/{brand}/LetterDetection/data/{part}/{part}_2_need_已鉴定再旋转'
    dst_img_dir = f'/media/D_4TB/YL_4TB/BaoDetection/data/{brand}/LetterDetection/data/{part}/{part}_2_need_已鉴定标记_包括标签旋转后'
    if os.path.exists(dst_img_dir):
        shutil.rmtree(dst_img_dir)
    os.mkdir(dst_img_dir)

    src_xml_dir = src_img_dir
    dst_xml_dir = dst_img_dir
