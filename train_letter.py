    ...
    if False:        # 存储第一批数据增强后的数据
        ...
        for i in range(len(dataset)):
            ...
            for label in labels:
                ...
                img = cv2.putText(img, str(n), (x1, y1), font, 1.2, (255, 255, 255), 2)
            plt.imsave(os.path.join(display_image_dir, f't{i}.jpg'), img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--brand', type=str, default='Chanel', help='brand')
    parser.add_argument('--part', type=str, default='sign', help='brand')
    ...
    # python -m torch.distributed.launch --nproc_per_node 2 train_YL.py --batch 64 --epochs 600 --brand Prada --weights yolov5s.pt --device 0,1
    # python -m torch.distributed.launch --nproc_per_node 2 train_YL.py -b 80 --epochs 600 --brand Prada --weights yolov5s.pt --device 0,1

    # Set DDP variables
    opt.total_batch_size = opt.batch_size
    opt.data = f'data/{opt.brand}_{opt.part}.yaml'
    opt.data_dir = f'../data/{opt.brand}/LetterDetection/data/{opt.part}'
    ...
    if opt.resume:  # resume an interrupted run
        ckpt = opt.resume if isinstance(opt.resume, str) else get_latest_run(str(Path(opt.data_dir).parent/'yolov5_rundata/train' )) # specified or most recent path
